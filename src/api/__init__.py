...
# default static_folder='static',template_folder='templates'
app = Quart(__name__)
...
async def setup() -> None:
    ...
    if not connect_string:
        ...
        sys_exit(1)
    L_DATA = Ldata(connect_string)
    if not await L_DATA.connect_pool():
        print("unable to create pool, exiting")
        sys_exit(2)
    if not await L_DATA.check_connection():
        print("unable to verify connection, exiting")
        sys_exit(3)
    if not await L_DATA.check_table():
        print("no expected table found, creating")
        if not await L_DATA.create_table():
            print("failed on table creation, exiting")
            sys_exit(4)
        if not await L_DATA.add_indexes():
...
async def serve_rendered_html() -> str:
    ...
    global L_DATA
    cnt = await L_DATA.count_all()
    print(cnt)
    ret = await L_DATA.select_all_limit_offset(0, 0)
...
async def check_db_health() -> DbHealth:
    ...
    global L_DATA
    if await L_DATA.check_connection():
...
async def get_memos(query_args: MemoListIn) -> MemoListOut:
    ...
    if query_args.nodata:
        ret = await L_DATA.count_all()
        return MemoListOut(count=ret, limit=0, offset=0, items=[])
    ret = await L_DATA.select_all_limit_offset(query_args.limit, query_args.offset)
    ret_list = []
    for item in ret:
...
async def add_memo_versioned(data: MemoIn) -> MemoOut:
    """add new memo"""
    global L_DATA
    ret = await L_DATA.insert_one(data.memo)
...
async def delete_memo_2(uuid) -> MemoDelOut:
    ...
    ret = await L_DATA.delete_one(uuid)
...
async def get_memo(uuid) -> MemoGetOut:
    ...
    try:
        ...
        return MemoGetOut(memo="invalid uuid", uuid=uuid), 400
    ret = await L_DATA.select_id(uuid)
